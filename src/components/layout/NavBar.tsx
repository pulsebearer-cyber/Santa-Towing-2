...
export default function NavBar() {
  ...
  const services = [
    'Emergency Towing', 'Vehicle Recovery', 'Car Rental', 'Vehicle Sales',
      ...
      <div className="max-w-7xl mx-auto px-4 md:px-8 flex justify-between items-center">
        ...
        <Link to="/" className="flex items-center gap-2 group">
          <img 
            ...
            className="h-10 md:h-11 w-auto object-contain bg-white p-1 rounded-[1px] shadow-sm"
            onError={(e) => {
          ...
          <div className="relative group cursor-pointer">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-52 bg-white text-dark rounded-[1px] shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 overflow-hidden border border-gray-100 p-1.5 z-50">
              {services.map(s => (
                <Link 
                  key={s} 
                  to={s === 'Emergency Towing' ? '/towing' : s === 'Car Rental' ? '/rental' : s === 'Vehicle Sales' ? '/sales' : s === 'Auto Repairs' ? '/repairs' : s === 'Body Works' ? '/body-works' : s === 'Auto Parts' ? '/parts' : s === 'Fleet Services' ? '/fleet' : '/services'} 
                  className="block px-3.5 py-2.5 hover:bg-slate-50 hover:text-accent rounded-[1px] transition-colors text-sm font-medium"
                >
                  {s}
                </Link>
              ))}
        ...
        <nav className="hidden lg:flex xl:hidden items-center gap-5 font-medium text-sm">
          ...
          <div className="relative group cursor-pointer">
            ...
            <div className="absolute top-full left-1/2 -translate-x-1/2 mt-1 w-48 bg-white text-dark rounded-[1px] shadow-xl opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-200 overflow-hidden border border-gray-100 p-1.5 z-50">
              <Link to="/sales" className="block px-3.5 py-2.5 hover:bg-slate-50 hover:text-accent rounded-[1px] transition-colors text-sm font-medium">Cars for Sale</Link>
              <Link to="/rental" className="block px-3.5 py-2.5 hover:bg-slate-50 hover:text-accent rounded-[1px] transition-colors text-sm font-medium">Rental Fleet</Link>
            </div>
          </div>
        ...
        <div className="flex items-center gap-3">
          <a 
            href="tel:0244753849" 
            className="hidden sm:inline-flex items-center gap-2 bg-accent hover:bg-accent/90 text-white px-5 py-2.5 rounded-[1px] font-semibold text-sm transition-all shadow-md shadow-accent/25 hover:shadow-accent/40 hover:-translate-y-0.5"
          >
            <Phone className="w-4 h-4" />
            <span>Call Now: 0244753849</span>
          </a>
          <button 
            className="lg:hidden p-2 text-inherit rounded-[1px] hover:bg-white/10 transition-colors"
        ...
        {mobileMenuOpen && (
          ...
          >
            <div className="flex flex-col p-4 font-medium divide-y divide-gray-100 max-h-[calc(100vh-80px)] overflow-y-auto">
              <Link to="/" className="py-3 px-4 hover:text-accent hover:bg-slate-50 rounded-[1px] transition-colors shrink-0">Home</Link>
              
              {/* Services Accordion */}
              <div className="flex flex-col shrink-0">
                <button 
                  onClick={() => setMobileServicesOpen(!mobileServicesOpen)}
                  className="flex justify-between items-center py-3 px-4 hover:text-accent hover:bg-slate-50 rounded-[1px] transition-colors w-full text-left"
                >
                  <span>Services</span>
                ...
                <AnimatePresence>
                  {mobileServicesOpen && (
                    ...
                    >
                      <Link to="/services" className="py-2.5 px-6 hover:text-accent text-sm font-medium">All Services</Link>
                      <Link to="/towing" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Emergency Towing</Link>
                      <Link to="/repairs" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Auto Repairs</Link>
                      <Link to="/body-works" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Body Works</Link>
                      <Link to="/parts" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Auto Parts</Link>
                      <Link to="/fleet" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Fleet Services</Link>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
                ...
                <AnimatePresence>
                  {mobileVehiclesOpen && (
                    <motion.div 
                      ...
                      className="overflow-hidden flex flex-col bg-slate-50 rounded-[1px] my-1 py-1"
                    >
                      <Link to="/sales" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Cars for Sale</Link>
                      <Link to="/rental" className="py-2.5 px-6 hover:text-accent text-sm font-medium">Rental Fleet</Link>
                    </motion.div>
                  )}
                </AnimatePresence>
              </div>
              ...
              <Link to="/contact" className="py-3 px-4 hover:text-accent hover:bg-slate-50 rounded-[1px] transition-colors shrink-0">Contact</Link>
              
              <div className="pt-4 pb-2 px-2 shrink-0">
                <a 
                  href="tel:0244753849" 
                  onClick={() => setMobileMenuOpen(false)} 
                  className="w-full bg-accent hover:bg-accent/90 text-white px-6 py-3.5 rounded-[1px] font-semibold transition-all text-center flex items-center justify-center gap-2 shadow-md"
                >
                  <Phone className="w-4 h-4" />
                  <span>Call Now: 0244753849</span>
                </a>
