  ...
  return (
    <main className="pt-24 lg:pt-[104px] min-h-screen bg-white">
          ...
          <div className="max-w-2xl">
            ...
            >
              <button 
                ...
                className="bg-accent hover:bg-accent/90 text-white px-8 py-4 rounded-[1px] font-bold transition-all shadow-lg shadow-accent/20"
              >
                View Open Positions
              </button>
              <a 
                href="#culture"
                className="bg-white/10 hover:bg-white/20 backdrop-blur-md border border-white/20 text-white px-8 py-4 rounded-[1px] font-bold transition-all"
              >
        ...
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
            {PERKS.map((perk, idx) => {
              ...
              return (
                <div key={idx} className="bg-white rounded-[1px] p-8 border border-dark/10 hover:shadow-xl transition-all hover:-translate-y-1 group">
                  <div className="w-14 h-14 bg-white rounded-[1px] shadow-sm flex items-center justify-center mb-6 group-hover:bg-accent transition-colors">
            ...
            <AnimatePresence>
                  ...
                  >
                    ...
                    <div className="shrink-0 flex items-center gap-4">
                      {expandedJobId !== job.id && (
                        <button 
                          className="bg-primary hover:bg-black text-white px-6 py-2.5 rounded-[1px] font-bold transition-all hidden sm:block"
                          onClick={(e) => {
                            e.stopPropagation();
                      ...
                      <div className={cn(
                        "w-10 h-10 rounded-[1px] border border-dark/20 flex items-center justify-center transition-transform",
                            ...
                            <div className="bg-white p-6 rounded-[1px] border border-dark/20 shadow-sm sticky top-32">
                              ...
                              {!isApplyingFor || isApplyingFor !== job.id ? (
                                <button 
                                  ...
                                  className="w-full bg-accent hover:bg-accent/90 text-white px-6 py-4 rounded-[1px] font-bold transition-all shadow-lg flex justify-center items-center gap-2 text-lg"
                                >
                    ...
                    {isApplyingFor === job.id && (
                      ...
                      >
                        <div className="p-6 md:p-10 border-t border-white/20 relative">
                          ...
                          <p className="text-dark/70 mb-8">Please fill out the form below to submit your application.</p>
                          
                          {applicationSuccess ? (
                            <div className="bg-primary/50/20 border border-primary/50 rounded-[1px] p-8 text-center">
                              <div className="w-16 h-16 bg-primary/50 rounded-[1px] flex items-center justify-center mx-auto mb-4">
                            ...
                            <form onSubmit={handleApply} className="space-y-6">
                              <div className="grid md:grid-cols-2 gap-6">
                                <div className="space-y-2">
                                  ...
                                  <input type="text" required className="w-full bg-white/5 border border-white/10 rounded-[1px] px-4 py-3 text-white focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                                </div>
                                <div className="space-y-2">
                                  <label className="text-sm font-bold text-white/70">Email Address *</label>
                                  <input type="email" required className="w-full bg-white/5 border border-white/10 rounded-[1px] px-4 py-3 text-white focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                                </div>
                                <div className="space-y-2">
                                  <label className="text-sm font-bold text-white/70">Phone Number *</label>
                                  <input type="tel" required className="w-full bg-white/5 border border-white/10 rounded-[1px] px-4 py-3 text-white focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                                </div>
                                <div className="space-y-2">
                                  <label className="text-sm font-bold text-white/70">Portfolio / LinkedIn URL</label>
                                  <input type="url" className="w-full bg-white/5 border border-white/10 rounded-[1px] px-4 py-3 text-white focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent" />
                                </div>
                              </div>
                              
                              <div className="space-y-2">
                                <label className="text-sm font-bold text-white/70">Resume / CV *</label>
                                <div className="border-2 border-dashed border-white/20 rounded-[1px] p-8 text-center hover:border-accent/50 transition-colors bg-white/5">
                                  <Paperclip className="w-8 h-8 text-dark/70 mx-auto mb-3" />
                                  <p className="text-sm text-white/70 mb-1">Drag and drop your resume here or</p>
                                  <button type="button" className="text-accent font-bold text-sm hover:underline">Browse files</button>
                                  <p className="text-xs text-dark/70 mt-2">PDF, DOCX up to 5MB</p>
                                </div>
                              </div>

                              <div className="space-y-2">
                                <label className="text-sm font-bold text-white/70">Cover Letter / Message</label>
                                <textarea rows={4} className="w-full bg-white/5 border border-white/10 rounded-[1px] px-4 py-3 text-white focus:outline-none focus:border-accent focus:ring-1 focus:ring-accent resize-none" placeholder="Tell us why you're a great fit for this role..."></textarea>
                              </div>

                              <button 
                                type="submit" 
                                disabled={isSubmitting}
                                className="bg-accent hover:bg-accent/90 text-white font-bold px-8 py-4 rounded-[1px] transition-all shadow-lg flex items-center justify-center gap-2 w-full md:w-auto disabled:opacity-50"
                              >
              ...
              {filteredJobs.length === 0 && (
                <div className="text-center py-12 bg-white rounded-[1px] border border-dark/20">
                  <div className="w-16 h-16 bg-white rounded-[1px] flex items-center justify-center mx-auto mb-4">
      ...
      <section className="py-20 bg-white border-y border-dark/20">
        <div className="max-w-7xl mx-auto px-4 md:px-8">
          ...
          <div className="relative">
            {/* Timeline line */}
            <div className="hidden md:block absolute top-1/2 left-0 right-0 h-1 bg-white -translate-y-1/2 z-0" />
            
            <div className="grid grid-cols-1 md:grid-cols-5 gap-8">
              {PROCESS_STEPS.map((step, idx) => (
                <div key={idx} className="relative z-10 flex flex-col items-center text-center">
                  <div className="w-16 h-16 bg-white rounded-[1px] border-4 border-dark/10 shadow-md flex items-center justify-center text-xl font-bold text-dark mb-4">
                    {idx + 1}
                  </div>
      ...
      <section className="py-20 bg-primary text-white text-center relative overflow-hidden">
        <div className="absolute top-0 right-0 w-64 h-64 bg-primary/20 blur-[100px] rounded-[1px]" />
        <div className="absolute bottom-0 left-0 w-64 h-64 bg-accent/10 blur-[100px] rounded-[1px]" />
        ...
        <div className="max-w-3xl mx-auto px-4 relative z-10">
          ...
          <div className="flex flex-wrap justify-center gap-4">
            <button 
              onClick={scrollToJobs}
              className="bg-accent hover:bg-accent/90 text-white px-8 py-4 rounded-[1px] font-bold transition-all shadow-xl text-lg"
            >
              View Open Positions
            </button>
            <Link to="/contact" className="bg-white/10 hover:bg-white/20 border border-white/20 text-white px-8 py-4 rounded-[1px] font-bold transition-all shadow-xl text-lg">
